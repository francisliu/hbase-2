...
public class TestClusterId {
  ...
  public void testClusterId() throws Exception  {
    ...
    ClusterId clusterId = new ClusterId(TEST_UTIL.getMiniHBaseCluster().getMaster().getZooKeeperWatcher(), null);
    assertTrue(clusterId.hasId());
    assertEquals(clusterId.getId(), rst.getRegionServer().getConfiguration().get(HConstants.CLUSTER_ID));
  }
}
