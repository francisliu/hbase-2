...
public class TestServerHealthChecker {
  
  private static final Log LOG = LogFactory.getLog(TestServerHealthChecker.class);
  ...
  public void testHealthChecker() throws Exception {   
    ...
    long timeout = config.getLong(HConstants.RS_HEALTH_SCRIPT_TIMEOUT, 100);
    ...
    checker.init("", timeout);
    assertFalse(checker.shouldRun());
  ...
  public void testRSHealthChore() throws Exception{
    ...
    String errorScript = "echo ERROR\n echo \"Server not healthy\"";
    createScript(errorScript, true);
    RegionServerHealthCheckChore rsChore = new RegionServerHealthCheckChore(100, stop, conf);
  ...
  private Configuration getConfForNodeHealthScript() {
    ...
    conf.set(HConstants.RS_HEALTH_SCRIPT_LOC,
      healthScriptFile.getAbsolutePath());
    conf.setLong(HConstants.RS_HEALTH_FAILURE_THRESHOLD, 3);
    conf.setLong(HConstants.RS_HEALTH_SCRIPT_TIMEOUT, 100);
    return conf;
  }
